...
ttg <- read.table('annotation/ttg.tsv', 
                  sep = '\t',
                  header = T,
                  stringsAsFactors = F
)
...
gsym <- read.table('annotation/gsym.tsv', 
                   sep = '\t',
                   header = T,
                   stringsAsFactors = F
)
...
annot.herv <- read.table('annotation/HERV_rmsk.hg38.v2.tsv', sep='\t', 
                         header=T, stringsAsFactors=F)
annot.l1 <- read.table('annotation/L1Base.hg38.v1.tsv', sep='\t',
                       header=T, stringsAsFactors=F)
...
save(ttg, gsym, retro.annot, retro.fam, file='analysis/01-load_gene.Rdata')
