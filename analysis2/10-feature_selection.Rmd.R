...
#         filter_at(vars(c('lrt','lasso','boruta')), ~.==1) %>%
#         .$rowid
# mat.sel <- mat[n,]
# 
# rres <- svm.cross(t(mat.sel), samples$clust.herv, k=5, nrep=100)
# svm.acc <- sapply(unlist(rres, recursive = F), function(l) l[[3]]$overall['Accuracy'])
# mean(svm.acc)
...
for(n in names(selected_vars)) {
  ...
  sel <- c('lrt', 'lasso')
  unsel <- setdiff(names(binmat), sel)
